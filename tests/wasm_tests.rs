...
async fn test_wasm_put_get() {
    ...
    })).await.expect("Failed to put value");
...
async fn test_wasm_history_and_time_travel() {
    ...
    // Store updated value
    db.put("docs", "readme", json!({"version": 2, "content": "Hello World"}))
        .await
        .expect("Failed to put updated value");
    
    // Get history
    let history = db.history("docs", "readme").await.expect("Failed to get history");
...
async fn test_wasm_namespaces() {
    ...
    assert!(namespaces.contains(&"users".to_string()), "Should have 'users' namespace");
    assert!(namespaces.contains(&"config".to_string()), "Should have 'config' namespace");
    
    // List keys in namespace
    let user_keys = db.list_keys("users").await;
    assert!(user_keys.contains(&"alice".to_string()), "Should have 'alice' key");
    assert!(user_keys.contains(&"bob".to_string()), "Should have 'bob' key");
    
    // Keys should be isolated by namespace
    let config_keys = db.list_keys("config").await;
    assert!(config_keys.contains(&"theme".to_string()), "Should have 'theme' key");
    assert!(!config_keys.contains(&"alice".to_string()), "Should not have 'alice' in config");
}
...
async fn test_wasm_vector_search() {
    ...
    db.store_vector("doc1", vec1.clone()).await.expect("Failed to store vector");
    db.store_vector("doc2", vec2.clone()).await.expect("Failed to store vector");
    db.store_vector("doc3", vec3.clone()).await.expect("Failed to store vector");
    
    // Search for similar vectors
    let query = Vector::new(vec![1.0, 0.0, 0.0], None, "test");
    let results = db.search_vectors(&query, 2).await.expect("Failed to search vectors");
...
fn test_wasm_causal_storage_direct() {
    ...
    // Put a value
    storage.put("test", "key1", json!("value1")).expect("Failed to put");
    ...
    // Update the value
    storage.put("test", "key1", json!("value2")).expect("Failed to put");
    
    // Get history
    let history = storage.history("test", "key1").expect("Failed to get history");
...
async fn test_wasm_query_engine() {
    ...
    // Insert test data
    db.put("products", "p1", json!({"name": "Laptop", "price": 999, "category": "electronics"})).await.unwrap();
    db.put("products", "p2", json!({"name": "Mouse", "price": 29, "category": "electronics"})).await.unwrap();
    db.put("products", "p3", json!({"name": "Desk", "price": 300, "category": "furniture"})).await.unwrap();
    
    // Query with filter
    let query = koru_delta::Query::new()
        .filter(koru_delta::Filter::Eq("category".to_string(), json!("electronics")));
...
async fn test_wasm_views() {
    ...
    // Insert test data
    db.put("sales", "s1", json!({"product": "A", "amount": 100})).await.unwrap();
    db.put("sales", "s2", json!({"product": "B", "amount": 200})).await.unwrap();
    db.put("sales", "s3", json!({"product": "A", "amount": 150})).await.unwrap();
    
    // Create a view
    let view_def = koru_delta::ViewDefinition::new()
        .aggregate(koru_delta::Aggregation::Sum("amount".to_string()));
    
    db.create_view("total_sales", "sales", view_def).await.expect("Failed to create view");
    
    // Query the view
    let view_data = db.query_view("total_sales").await.expect("Failed to query view");
