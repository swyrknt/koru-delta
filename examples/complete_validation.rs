...
async fn main() {
    ...
    print!("[3.5] List keys... ");
    for i in 0..50 {
        db.put("listtest", &format!("key{}", i), json!(i))
            .await
            .unwrap();
    }
    ...
    for i in 0..1000 {
        db.put("big_ns", &format!("k{}", i), json!(i))
            .await
            .unwrap();
    }
    ...
    println!("✅ ({} matching)", r.records.len());
    passed += 1;
    total_tests += 1;
    ...
    let v = db
        .embed(
            "vec",
            "doc1",
            koru_delta::vector::Vector::synthesize(&json!({"text": "hello"}), 128),
            None,
        )
        .await;
    assert!(v.is_ok());
    ...
    println!("✅ (dim: {})", vec.dimensions());
    passed += 1;
    total_tests += 1;

    // 6.3 Put similar (simplified)
    print!("[6.3] Put similar (simplified)... ");
    db.put_similar("vecs", "doc2", json!({"content": "test"}), None)
        .await
        .unwrap();
    println!("✅");
    passed += 1;
    ...
    for i in 0..10 {
        db.put_similar("vecs", &format!("doc{}", i), json!({"id": i}), None)
            .await
            .unwrap();
    }
    println!("✅");
    ...
    let results = db
        .find_similar(Some("vecs"), json!({"content": "test"}), 5)
        .await
        .unwrap();
    ...
    db.put("ws1", "data", json!({"project": "alpha"}))
        .await
        .unwrap();
    ...
    println!("✅ (id: {}...)", &identity.public_key[..16]);
    passed += 1;
    total_tests += 1;
    ...
    for _ in 0..100 {
        ...
        handles.push(tokio::spawn(
            async move { db.get("concurrent", "k50").await },
        ));
    }
    for h in handles {
        h.await.unwrap().unwrap();
    ...
    for _ in 0..5 {
        let db = db.clone();
        handles.push(tokio::spawn(
            async move { db.refresh_view("elec-view").await },
        ));
    }
    for h in handles {
        h.await.unwrap().unwrap();
    ...
    println!("✅ ({} ns)", stats.namespace_count);
    passed += 1;
    total_tests += 1;
    ...
    let _ = db
        .synthesize_storage_action(StorageAction::Query {
            pattern_json: json!({}),
        })
        .await;
    println!("✅");
    ...
    if failed > 0 {
        ...
        println!(
            "\n✨ ALL {} TESTS PASSED! Database is fully validated.",
            passed
        );
        println!("\nFeatures validated:");
