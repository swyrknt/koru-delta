...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "{}",
        "║     CRISIS COORDINATION SYSTEM - KoruDelta Demo               ║"
            .bold()
            .cyan()
    );
    println!(
        "{}",
        "║     Full Feature Showcase with Real-World Scenario            ║"
            .bold()
            .cyan()
    );
    println!(
    ...
    println!(
        "\n{}",
        "📦 PHASE 1: Initializing KoruDelta Database"
            .bold()
            .yellow()
    );
    ...
    db.put("identities", "fire-dept", json!(fire_id.identity))
        .await?;
    db.put("identities", "police-dept", json!(police_id.identity))
        .await?;
    db.put("identities", "medical-dept", json!(medical_id.identity))
        .await?;
    ...
    println!(
        "   Creating {} incidents with causal timestamps...",
        incidents.len()
    );
    ...
    for (id, desc) in &descriptions {
        ...
        db.embed("vectors", *id, embedding, Some(json!({"desc": desc})))
            .await?;
    }
    ...
    println!(
        "   ✓ Incidents restored: {}",
        db2.list_keys("incidents").await.len()
    );
