...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "{}",
        "║     STRESS TEST - Load & Edge Case Validation                 ║"
            .bold()
            .cyan()
    );
    println!(
    ...
    println!(
        "\n{}",
        "🚀 TEST 1: High-Throughput Writes (1000 ops)"
            .bold()
            .yellow()
    );
    ...
    let start = Instant::now();
    for i in 0..10_000 {
        db.put("many-keys", &format!("key-{}", i), json!({"n": i}))
            .await?;
    }
    let elapsed = start.elapsed();
    ...
    assert!(
        history.len() >= 50,
        "Should have significant history: {}",
        history.len()
    );
