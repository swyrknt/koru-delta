...
impl TaskFlow {
    ...
    async fn add_project_member(&self, project_id: &str, user_id: &str) -> anyhow::Result<()> {
        ...
        if !members.contains(&json!(user_id)) {
            ...
            self.db
                .put("projects", project_id, json!(updated_obj))
                .await?;
    ...
    async fn update_task_status(&self, task_id: &str, status: &str) -> anyhow::Result<()> {
        ...
        self.db
            .put("tasks", task_id, json!(task_obj.clone()))
            .await?;
    ...
    async fn find_similar_tasks(
        ...
        let results = self
            .db
            .find_similar(Some("task_embeddings"), json!({"text": query}), top_k)
            .await?;

        let tasks: Vec<(String, f32)> = results.into_iter().map(|r| (r.key, r.score)).collect();
    ...
    async fn get_high_priority_tasks(&self) -> anyhow::Result<Vec<serde_json::Value>> {
        let query = Query {
            filters: vec![Filter::eq("priority", "high"), Filter::eq("status", "todo")],
            limit: Some(20),
            ..Default::default()
        };
    ...
    async fn get_user_workload(&self, user_id: &str) -> anyhow::Result<serde_json::Value> {
        ...
        let high_priority = active_tasks
            .iter()
...
async fn main() -> anyhow::Result<()> {
    ...
    let pm_id = taskflow
        .create_user("Alice Chen", "alice@taskflow.io", "product_manager")
        .await?;
    println!("✅");
    passed += 1;

    print!("Creating tech lead... ");
    let tech_lead_id = taskflow
        .create_user("Bob Smith", "bob@taskflow.io", "tech_lead")
        .await?;
    println!("✅");
    passed += 1;

    print!("Creating developers... ");
    let dev1_id = taskflow
        .create_user("Carol Jones", "carol@taskflow.io", "developer")
        .await?;
    let dev2_id = taskflow
        .create_user("David Lee", "david@taskflow.io", "developer")
        .await?;
    println!("✅ (2 devs)");
    passed += 1;

    print!("Creating designer... ");
    let designer_id = taskflow
        .create_user("Eve Wang", "eve@taskflow.io", "designer")
        .await?;
    println!("✅");
    passed += 1;
    ...
    taskflow
        .add_project_member(&mobile_proj, &tech_lead_id)
        .await?;
    taskflow.add_project_member(&mobile_proj, &dev1_id).await?;
    taskflow
        .add_project_member(&mobile_proj, &designer_id)
        .await?;
    println!("✅ (3 members added)");
    passed += 1;
    ...
    taskflow
        .update_task_status(&mobile_tasks[0], "in_progress")
        .await?;
    taskflow
        .update_task_status(&mobile_tasks[1], "in_progress")
        .await?;
    taskflow
        .update_task_status(&mobile_tasks[2], "done")
        .await?;
    taskflow
        .update_task_status(&web_task_ids[0], "in_progress")
        .await?;
    println!("✅ (4 status updates)");
    passed += 1;
    ...
    println!("✅ ({} versions tracked)", task_history.len());
    passed += 1;
    ...
    println!(
        "✅ ({} tasks in mobile project)",
        mobile_project_tasks.len()
    );
    passed += 1;
    ...
    println!("✅ ({} tasks assigned)", dev1_tasks.len());
    passed += 1;
    ...
    println!("✅ (2 views created)");
    passed += 1;

    print!("Querying views... ");
    let view_results = taskflow
        .db
        .query_view(&format!("{}_tasks", mobile_proj))
        .await?;
    assert_eq!(view_results.records.len(), 10);
    println!("✅ ({} records in view)", view_results.records.len());
    passed += 1;
    ...
    let ai_tasks = taskflow
        .find_similar_tasks("artificial intelligence machine learning", 3)
        .await?;
    println!("✅ (found {} related tasks)", ai_tasks.len());
    passed += 1;

    print!("Finding similar tasks (UI-related)... ");
    let ui_tasks = taskflow
        .find_similar_tasks("user interface design", 3)
        .await?;
    println!("✅ (found {} related tasks)", ui_tasks.len());
    passed += 1;

    print!("Finding similar tasks (performance)... ");
    let perf_tasks = taskflow.find_similar_tasks("optimization speed", 3).await?;
    println!("✅ (found {} related tasks)", perf_tasks.len());
    passed += 1;
    ...
    println!(
        "✅ Mobile: {} total, {} done",
        mobile_stats["total_tasks"], mobile_stats["done"]
    );
    passed += 1;
    ...
    println!(
        "✅ Web: {} total, {} done",
        web_stats["total_tasks"], web_stats["done"]
    );
    passed += 1;
    ...
    println!(
        "✅ Dev1: {} tasks, workload score {}",
        dev1_workload["total_assigned"], dev1_workload["workload_score"]
    );
    passed += 1;
    ...
    println!("✅ ({} users stored)", all_users.len());
    passed += 1;
